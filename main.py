...
from app import load_json, load_default_config, load_env, deep_merge_conflict, setup_logger
...
def main():
 ...
 if args.env is None:
  ...
  timeout_config = default_config["api"]["timeout"]
  geo_data = fetch_geo_location(geo_location_url, api_weather_key, city_name, state_code, country_code, timeout_config)[0]
  geo_lat = geo_data["lat"]
  geo_lon = geo_data["lon"]

  weather_url = default_config["api"]["weather_url"]
  weather_data = fetch_current_weather_data(weather_url, api_weather_key, geo_lat, geo_lon, timeout_config)
  print(weather_data)
 elif args.env:
  load_env(args.env)
  override_env_config = load_json(args.env)
  merge_config = deep_merge_conflict(default_config, override_env_config)
  setup_logger(merge_config)
  api_weather_key = os.getenv("API_KEY_WEATHER")
  ...
  timeout_config = merge_config["api"]["timeout"]
  geo_data = fetch_geo_location(geo_location_url, api_weather_key, city_name, state_code, country_code, timeout_config)[0]
  geo_lat = geo_data["lat"]
  geo_lon = geo_data["lon"]
